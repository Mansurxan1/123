  ...
  reducers: {
    setShops: (state, action) => {
      ...
      if (state.selectedBranch) {
        const currentBranch = action.payload.find((shop) => shop.id === state.selectedBranch.id)

        if (!currentBranch || !currentBranch.is_active || currentBranch.work_status === "yopiq") {
          // Yangi ochiq filial topish
          const newBranch = action.payload.find((shop) => shop.is_active && shop.work_status === "ochiq")
